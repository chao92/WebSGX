...
void Reference::loadFromFASTA (const string &fn)
{
	...
	{ // Try loading .fai file
		...
		if (fastaidx == nullptr)
		{
			printf("Cannot open FASTA index %s", faidx.c_str());
			abort();
		}
//			throw DZException("Cannot open FASTA index %s", faidx.c_str());
		...
		while (fscanf((FILE*) fastaidx->handle(), "%s %lu %lu %*lu %*lu", chr, &len, &loc) != EOF) 
			CHROM_UPDATE(chromosomes[chr], chr, "", fn, len, loc);
	...
	catch (DZException &e) 
	{
		...
		while ((c = input->getc()) != EOF) {
			if (c == '>') {
				...
				pos = input->tell();
				CHROM_UPDATE(chromosomes[chr], chr, "", filename, 0, pos);
				continue;
			}
			if (!isspace(c)) 
				cnt++;
...
std::string Reference::scanChromosome (const string &s) //, const SAMComment &samComment) 
{
	...
	try {
		if (directory == "") {
			...
			if (it == chromosomes.end()) 
			{
				printf("Chromosome %s not found in the reference", s.c_str());
				abort();
			}
//				throw DZException("Chromosome %s not found in the reference", s.c_str());
		...
		try {
			...
			filename = "";
			CHROM_UPDATE(chromosomes[s], s, "", "<implicit>", 0, 0);
...
char Reference::getCharAt(size_t pos) const
{
