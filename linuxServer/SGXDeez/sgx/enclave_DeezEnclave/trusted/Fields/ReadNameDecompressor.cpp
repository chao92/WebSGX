...
ReadNameDecompressor::ReadNameDecompressor (int blockSize):
	...
	paired(blockSize)
{
	streams.resize(ReadNameDecompressor::Fields::ENUM_COUNT);
	if (optBzip) {
...
void ReadNameDecompressor::importRecords (uint8_t *in, size_t in_size) 
{
	...
	Array<uint8_t> index;
	size_t s1 = decompressArray(streams[ReadNameDecompressor::Fields::INDEX], in, index);
	Array<uint8_t> content;
	decompressArray(streams[ReadNameDecompressor::Fields::CONTENT], in, content);
	
	Array<uint8_t> pp;
	decompressArray(streams[ReadNameDecompressor::Fields::PAIRED], in, pp);
	uint8_t *p = pp.data();
	paired.resize(0);
...
{
	streams[ReadNameDecompressor::Fields::CONTENT]->setCurrentState(in, in_size);
}