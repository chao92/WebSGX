#ifndef DEEZENCLAVE_T_H__
#define DEEZENCLAVE_T_H__
...
void useEnclaveGetFileSize(sgx_ra_context_t context, char* data, int data_size, char* decryptedData, int decryptedData_size, int numberofParties, int client_i);
void encryptData(sgx_ra_context_t context, uint8_t* p_src, uint32_t src_len, uint8_t* p_dst, uint32_t dst_len);
void ecall_decryped_seal_buffer(sgx_ra_context_t context, char* data2seal, int data_buffer_size, char* sealed_secret, int sealed_size, int true_size, int64_t index, char* sealedDataMAC, int mac_size);
void ecallHere(size_t inFileSz, int numFiles, char* sealedDataMAC, int mac_size);
...
sgx_status_t put_samfile_data(sgx_ra_context_t context, uint8_t* p_secret, uint32_t secret_size, uint8_t* gcm_mac);
sgx_status_t put_reffile_data(sgx_ra_context_t context, uint8_t* p_secret, uint32_t secret_size, uint8_t* gcm_mac);
sgx_status_t put_indexfile_data(sgx_ra_context_t context, uint8_t* p_secret, uint32_t secret_size, uint8_t* gcm_mac);
