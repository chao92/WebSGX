...
int Enclave_vasprintf(char **strp, const char *fmt, va_list ap)//changed by wwj
{
//    va_list ap1;
	size_t size;
	char *buffer;
...
DZException::DZException (const char *fmt, ...)
{
	char *ptr = 0;
	va_list args;
	va_start(args, fmt);
	Enclave_vasprintf(&ptr, fmt, args);
	va_end(args);
	std::string s = ptr;
	free(ptr);
...
DZSortedException::DZSortedException (const char *fmt, ...)
{
	char *ptr = 0;
	va_list args;
	va_start(args, fmt);
