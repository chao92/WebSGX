...
{
	streams.resize(PairedEndDecompressor::Fields::ENUM_COUNT);
	if (optBzip) {
...
void PairedEndDecompressor::importRecords (uint8_t *in, size_t in_size) 
{
	...
	size_t s = decompressArray(streams[PairedEndDecompressor::Fields::TLENBIT], in, tlenBits);
	decompressArray(streams[PairedEndDecompressor::Fields::DIFF], in, buffer);
	decompressArray(streams[PairedEndDecompressor::Fields::CHROMOSOME], in, chromosomes);
	decompressArray(streams[PairedEndDecompressor::Fields::TLEN], in, tlens);
	...
	map<char, std::string> chromosomeIndex;
	char chr;
