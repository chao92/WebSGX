...
EditOperationDecompressor::EditOperationDecompressor (int blockSize, const SequenceDecompressor &seq):
	...
	sequence(seq)
{
	streams.resize(EditOperationDecompressor::Fields::ENUM_COUNT);
	for (int i = 0; i < streams.size(); i++)
		streams[i] = new GzipDecompressionStream();
	if (optBzip) {
		streams[EditOperationDecompressor::Fields::OPCODES] = new BzipDecompressionStream();
		streams[EditOperationDecompressor::Fields::ACGT_U] = new BzipDecompressionStream();
	}
	streams[EditOperationDecompressor::Fields::LOCATION] = new ArithmeticOrder0DecompressionStream<256>();
}
...
void EditOperationDecompressor::importRecords (uint8_t *in, size_t in_size) 
{
	...
	decompressArray(streams[EditOperationDecompressor::Fields::STITCH], in, stitches);
	Array<uint8_t> locations;
	size_t sz = decompressArray(streams[EditOperationDecompressor::Fields::LOCATION], in, locations);

	REPEAT(3) {
		nucleotides[_] = ACTGStream();
		decompressArray(streams[EditOperationDecompressor::Fields::ACGT + _], in, nucleotides[_].seqvec);
		decompressArray(streams[EditOperationDecompressor::Fields::ACGT + _], in, nucleotides[_].Nvec);
		nucleotides[_].initDecode();
	}

	vector<Array<uint8_t>> oa(EditOperationDecompressor::Fields::ACGT);
...
inline EditOperation EditOperationDecompressor::getEditOperation (size_t loc, vector<uint8_t*> &fields) 
{
	...
	int count = getEncoded(fields[EditOperationDecompressor::Fields::OPLEN]) - 1;
	for (int i = 0; i < count; i++) {
		char c = *fields[EditOperationDecompressor::Fields::OPCODES]++;
		assert(c);
		int l = 0;
		if (c == '*') { // Unmapped case. Just add * and exit
			assert(count == 1);
			endPos = getEncoded(fields[EditOperationDecompressor::Fields::SEQEND]) - 1;
		...
		endPos += getEncoded(fields[EditOperationDecompressor::Fields::SEQPOS]) - 1;
		if (c == '0') {
			c = *fields[EditOperationDecompressor::Fields::OPCODES]++, l = 0; i++;
		} else if (c == 'X') {
			l = getEncoded(fields[EditOperationDecompressor::Fields::XLEN]);
		} else if (c == 'H' || c == 'S') {
			l = getEncoded(fields[EditOperationDecompressor::Fields::HSLEN]);
		} else {
			l = getEncoded(fields[EditOperationDecompressor::Fields::LEN]);
		}
    ...
    return eo; // fixed by Shuang Wang
	ZAMAN_END(GetEO);
}
...
EditOperation &EditOperationDecompressor::getRecord(size_t i) 
{
	...
	if (mdOperLen || !isdigit(*(eo.MD.end()-1))) inttostr(mdOperLen, eo.MD);
...
{
	streams[EditOperationDecompressor::Fields::LOCATION]->setCurrentState(in, in_size);
}
