...
if ($_POST) {
	...
	$ref_type = 5;
	socket_send($socket, pack("I",$ref_type), 4, MSG_WAITALL);
	...
	if (strcmp($out,"data")) {
		$out = 'reffile';// for debug use
		$ref_length=$_POST['length'];
		// $highMap = 0xffffffff00000000; 
		// $lowMap = 0x00000000ffffffff; 
		// $higher = ($msg4_length & $highMap) >>32; 
		// $lower = $msg4_length & $lowMap; 
		// $packed = pack('NN', $higher, $lower); 
		$packed_length = pack("I",$ref_length);

		socket_send($socket, $packed_length, 4, MSG_WAITALL);

		$ref_hexstring = $_POST['ref'];

		$ref_bin = hex2bin($ref_hexstring);

		socket_send($socket, $ref_bin, $ref_length, MSG_WAITALL);
	}
