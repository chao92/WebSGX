...
if ($_POST) {
	...
	$index_type = 6;
	socket_send($socket, pack("I",$index_type), 4, MSG_WAITALL);
	...
	if (strcmp($out,"data")) {
		$out = 'indexfile';
		$index_length=$_POST['length'];
		// $highMap = 0xffffffff00000000; 
		// $lowMap = 0x00000000ffffffff; 
		// $higher = ($msg4_length & $highMap) >>32; 
		// $lower = $msg4_length & $lowMap; 
		// $packed = pack('NN', $higher, $lower); 
		$packed_length = pack("I",$index_length);

		socket_send($socket, $packed_length, 4, MSG_WAITALL);

		$index_hexstring = $_POST['inx'];

		$index_bin = hex2bin($index_hexstring);

		socket_send($socket, $index_bin, $index_length, MSG_WAITALL);
	}


	$res = array('err1' => $err1, 'err2' => $err2, 'err3' => $err3, 'signal'=>$out, 'sendoutis' => $index_hexstring);
